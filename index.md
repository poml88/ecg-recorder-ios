...
The open source software [EDFbrowser](https://www.teuniz.net/edfbrowser) can convert the ascii text files into .edf files which can be nicely browsed using the software. Some [sample ECG data](/ecg-recorder-ios/ecg-examples/examples.html) and [/ecg-recorder-ios/demo videos](demo-video/videos.html) for import into Apple Numbers and EDFBrowser are available.
